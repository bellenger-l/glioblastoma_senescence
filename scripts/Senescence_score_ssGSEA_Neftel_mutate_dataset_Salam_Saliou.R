...
#Only mutated tumors for CD4 and CDKN2A
...
neftel_smartseq <- read.delim("./Inputs/GSE131928_RAW/GSM3828672_Smartseq2_GBM_IDHwt_processed_TPM.tsv", 
                              header = TRUE, 
...
neftel_smarseq_metadata <- read.delim("./Inputs/Neftel_SingleCellPortal/scAdultPediatricGlio/IDHwt.GBM.Metadata.SS2.txt", 
                                      check.names = FALSE,
                                      header = TRUE, 
                                      row.names = 1)
...
tumor_to_keep <- c("MGH128", "MGH136", "MGH143", "MGH151", "MGH152") #mutate for CD4 and/or CDKN2A
...
## QC
VlnPlot(neftel_smartseq_seurat, 
        features = c("nCount_RNA", "nFeature_RNA")) # QC plots

## Representation
DimPlot(neftel_smartseq_seurat, 
        reduction = "umap", 
        group.by = c("Sample"))


neftel_smartseq_seurat <- MetaModule_Assignment(neftel_smartseq_seurat, 
                                                MetaModule_gene_sets, 
                                                assay = "RNA")
DimPlot(neftel_smartseq_seurat, 
        split.by = c("Sample"), 
        group.by = c("state"))
...
#tumor color palette
neftel_colors <- brewer.pal(length(unique(neftel_smartseq_seurat$Sample)), "Set1")
...
neftel_SS2_ssGsea = gsva(as.matrix(neftel_smartseq_seurat@assays$RNA@data), list_geneset, 
...
#plots
plot1 <- FeaturePlot(neftel_smartseq_seurat, features = c("score_ssGSEA")) + 
...
write.table(neftel_SS2_ssGsea, 
            "./figures_neftel_10X_SS2_p16/neftel_SS2_ssGSEA.tsv", 
            quote = FALSE, sep = "\t")
...
ggplot(neftel_smartseq_seurat@meta.data, 
  ...
  geom_hline(yintercept = min(neftel_smartseq_seurat@meta.data$score_ssGSEA[neftel_smartseq_seurat@meta.data$rate_ssGSEA == "High"]),
             linetype="dashed") +
  ggtitle("Senescence score (ssGSEA) distribution between non-integrated patient datasets", "SS2 dataset")

#pie charts
neftel_SS2_summary <- subset(neftel_smartseq_seurat@meta.data, 
                             select = c("rate_ssGSEA", "state", "Sample"))
# by Module
neftel_SS2_summary_byRate <- dcast(neftel_SS2_summary, 
                                   rate_ssGSEA ~ state)
neftel_SS2_summary_byRate <- melt(neftel_SS2_summary_byRate, 
                                  id.vars = "rate_ssGSEA")
...
saveRDS(neftel_smartseq_seurat, 
        file = "./figures_neftel_10X_SS2_p16/neftel_smartseq_mutate_SeuratObject.rds")